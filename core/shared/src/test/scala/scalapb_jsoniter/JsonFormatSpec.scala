...
class JsonFormatSpec extends AnyFlatSpec with Matchers with OptionValues with JavaAssertions {
  ...
  "Empty object" should "give empty json" in {
    assertJsonEquals(JsonFormat.toJsonString(MyTest()), "{}")
  }

  "Empty object" should "give empty json for MyTest3" in {
    assertJsonEquals(JsonFormat.toJsonString(MyTest3()), "{}")
  }
  ...
  "Set treat" should "give correct json" in {
    assertJsonEquals(
      JsonFormat.toJsonString(MyTest(trickOrTreat = MyTest.TrickOrTreat.Treat(MyTest()))),
      """{"treat": {}}"""
    )
  }
  ...
  "JsonFormat" should "encode and decode enums for proto3" in {
    ...
    assertJsonEquals(JsonFormat.toJsonString(v1Value), """{"optEnum": "V1"}""")
    val defaultValue = MyTest3(optEnum = MyTest3.MyEnum3.UNKNOWN)
    assertJsonEquals(JsonFormat.toJsonString(defaultValue), """{}""")
  ...
  "TestProto" should "be TestJson when converted to Proto" in {
    assertJsonEquals(JsonFormat.toJsonString(TestProto), TestJson)
  }
  ...
  "Empty object" should "give full json if including default values" in {
    assertJsonEquals(
      new Printer(includingDefaultValueFields = true).print(MyTest()),
      """{
        |  "hello": "",
        |  "foobar": 0,
        |  "bazinga": "0",
  ...
  "Empty object" should "with preserve field names should work" in {
    assertJsonEquals(
      new Printer(includingDefaultValueFields = true, preservingProtoFieldNames = true).print(MyTest()),
      """{
        |  "hello": "",
        |  "foobar": 0,
        |  "bazinga": "0",
  ...
  "TestProto" should "produce valid JSON output for unsigned integers" in {
    val uint32max: Long = (1L << 32) - 1
    assertJsonEquals(JsonFormat.toJsonString(IntFields(uint = Some(uint32max.toInt))), s"""{"uint":$uint32max}""")
    assertJsonEquals(JsonFormat.toJsonString(IntFields(uint = Some(1))), s"""{"uint":1}""")
    assertJsonEquals(
      JsonFormat.toJsonString(IntFields(fixint = Some(uint32max.toInt))),
      s"""{"fixint":$uint32max}"""
    )
    assertJsonEquals(JsonFormat.toJsonString(IntFields(fixint = Some(1))), s"""{"fixint":1}""")
    val uint64max: BigInt = (BigInt(1) << 64) - 1
    assertJsonEquals(
      JsonFormat.toJsonString(IntFields(ulong = Some(uint64max.toLong))),
      s"""{"ulong":"$uint64max"}"""
    )
    assertJsonEquals(JsonFormat.toJsonString(IntFields(ulong = Some(1))), s"""{"ulong":"1"}""")
    assertJsonEquals(
      JsonFormat.toJsonString(IntFields(fixlong = Some(uint64max.toLong))),
      s"""{"fixlong":"$uint64max"}"""
    )
    assertJsonEquals(JsonFormat.toJsonString(IntFields(fixlong = Some(1))), s"""{"fixlong":"1"}""")
  }
  ...
  "DoubleFloatProto" should "parse NaNs" in {
    ...
    val json = parseJson(JsonFormat.toJsonString(out)).asInstanceOf[Map[String, Any]]
    json("d") must be("NaN")
    json("f") must be("NaN")
  }
  ...
  "DoubleFloatProto" should "parse Infinity" in {
    ...
    val json = parseJson(JsonFormat.toJsonString(out)).asInstanceOf[Map[String, Any]]
    json("d") must be("Infinity")
    json("f") must be("Infinity")
  }
  ...
  "TestProto packed as any" should "give TestJsonWithType after JSON serialization" in {
    ...
    assertJsonEquals(anyEnabledPrinter.print(any), TestJsonWithType)
  }

  "TestJsonWithType" should "be TestProto packed as any when parsed from JSON" in {
    val out = anyEnabledParser.fromJsonString[PBAny](TestJsonWithType)
  ...
  "formatEnumAsNumber" should "format enums as number" in {
    ...
    assertJsonEquals(new Printer(formattingEnumsAsNumber = true).print(p), s"""{"optEnum":2}""")
  }
  ...
  "TestProto" should "be TestJsonWithMapEntriesAsKeyValuePairs when converted to Proto with mapEntriesAsKeyValuePairs setting" in {
    assertJsonEquals(
      new Printer(formattingMapEntriesAsKeyValuePairs = true).print(TestProto),
