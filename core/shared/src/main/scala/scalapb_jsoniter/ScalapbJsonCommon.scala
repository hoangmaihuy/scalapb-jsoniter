...
// Vendored (trimmed to what scalapb-jsoniter uses) from scalapb-json-common
// (io.github.scalapb-json) so this library can compile against scalapb-runtime
// 1.0.0-alpha.x, which upstream does not yet target.
object ScalapbJsonCommon {
  def unsignedInt(n: Int): Long = n & 0x00000000ffffffffL

  type GenericCompanion = GeneratedMessageCompanion[? <: GeneratedMessage]
